function [Parameters] = LSCAN_getMembraneParameters(Variables, filenametext)
smoothy = 20;
...
if length(LHS_S_maxima)>1
    ...
    Parameters.Linescan.period(1,:)=nanmean(dt*abs(LHS_S_maxima-LHS_S_minima)*2);
    Parameters.Linescan.period(2,:)=nanmean(dt*abs(RHS_S_maxima-RHS_S_minima)*2);
end
